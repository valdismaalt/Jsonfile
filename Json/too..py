...
class TuntudEestiInimesed:
    ...
    def calculate_age(self, birth_date_str, death_date_str=None):
        birth_date = datetime.strptime(birth_date_str, "%Y-%m-%d").date()
        if death_date_str and death_date_str != '0000-00-00':
            reference_date = datetime.strptime(death_date_str, "%Y-%m-%d").date()
        else:
